...
function answerLabel(q){let n=required(q);return `${countWord(n)} EASY USCIS-ACCEPTED ${n===1?"ANSWER":"ANSWERS"}`}
...
function displayQuestion(){
 ...
 sectionPill.textContent=q.section;countPill.textContent=`Question ${q.id}/100`;qnum.textContent=`QUESTION ${q.id}`;question.textContent=q.q;answerLabel.textContent=answerLabelFn(q);
...
function answerLabelFn(q){return answerLabel(q)}
...
function startTest(){testOrder=[...Q].sort(()=>Math.random()-.5).slice(0,10).map(q=>q.id);testPos=0;testCorrect=0;testWrong=0;switchPanel("test");showTest()}
...
function showTest(){
 ...
 tAnswerLabel.textContent=answerLabel(q);tEasy.innerHTML=q.easy.map(x=>`<div>${escapeHtml(x)}</div>`).join("");let other=q.a.filter(a=>!isEasy(a,q));tOthers.innerHTML=other.map(x=>`<li>${escapeHtml(x)}</li>`).join("");tOtherBox.style.display=other.length?"none":"none";tInfo.textContent=q.info;
 [tAnswerLabel,tEasy,tOtherBox,tInfoBox].forEach(x=>x.style.display="none");revealBtn.style.display="block";resultBtns.classList.remove("show")
}
